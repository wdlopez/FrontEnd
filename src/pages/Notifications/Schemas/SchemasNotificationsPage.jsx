...
import AddSchemaNotificationModal from "../../../components/organisms/Forms/AddSchemaNotificationModal";
import Swal from "sweetalert2";
import { normalizeList } from "../../../utils/api-helpers";

const SchemasNotificationsPage = () => {
  ...
  const fetchSchemas = useCallback(async () => {
    setLoading(true);
    try {
      const response = await SchemaNotificationService.getAllSchemas({
  ...
  const handleDelete = async (id, clientKey) => {
    ...
    if (result.isConfirmed) {
      try {
        await SchemaNotificationService.deleteSchema(id);
  ...
  const handleValidate = async (id) => {
    try {
      ...
      const res = await SchemaNotificationService.validateSchema(id);
...
export default SchemasNotificationsPage;