...
const GenericEditModal = ({ 
  ...
  const formFields = config?.columns
    ?.filter(col => col.backendKey && col.editable !== false && col.backendKey !== 'id' && col.backendKey !== 'active' && col.backendKey !== 'status' && !col.hideInForm && col.type !== 'password' && col.backendKey !== 'roleId' && col.backendKey !== 'entityId' && col.backendKey !== 'providerId')
    .map(col => {
      const field = {
        name: col.backendKey,
        label: col.header,
